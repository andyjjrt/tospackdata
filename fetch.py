...
print(target_url)
monster_data = json.dumps(json5.loads(res.split("var t=new Map(")[1].split(".map((function(a){return[a.id,a]})))}")[0].replace("!", "")))
monster_active = json.dumps(json5.loads(res.split("var t=new Map(")[2].split(".map((function(a){return[a[0],{name:JSON.parse(a[1].name),description:JSON.parse(a[1].description)}]})))}")[0]))
monster_leader = json.dumps(json5.loads(res.split("var t=new Map(")[3].split(".map((function(a){return[a[0],{name:JSON.parse(a[1].name),description:JSON.parse(a[1].description)}]})))}")[0]))
monster_team = json.dumps(json5.loads(res.split("var t=new Map(")[4].split(".map((function(a){return[a[0],JSON.parse(a[1])]})))}")[0]))
